...
import { deleteItem, updateItem } from '../../redux/slices/cartSlice'; // Yeni yerden içe aktarım
...
const CartItem = ({ item }) => {
  ...
  // Miktarı 1 arttır
  const increase = () => {
    dispatch(updateItem({ id: item.id, newAmount: item.amount + 1 }));
  };
  ...
  return (
    <div className="flex gap-4 border mb-10 rounded-lg p-4 hover:shadow">
      <img className="w-[115px] rounded-lg" src={item.photo} alt={item.title} />
